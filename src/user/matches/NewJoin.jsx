...
const NewJoin = () => {
  ...
  const fetchData = async () => {
    if (userData && userData?.gender) {
      try {
        const partnerDetails = {...userData?.partnerPreference, gender : userData?.gender};
  ...
  return (
    ...
    {isLoading ? (
      ...
      <div className="px-96  w-full mt-5">
        ...
        </div></>  // Display a loading message while data is being fetched
    ) : matchData.length === 0 ? (
      <DataNotFound
      ...
      matchData.map((item, index) => (
        <Card item={item} key={index} fetchData={fetchData}/>
      ))
    )}
    </div>
  </>
